...
#pragma warning(pop)
	...
	virtual void Update() override
	{
		for (auto entity : m_Entities)
		{
			...
			if (m_CharacterWindows[entity].m_Visible)
			{
				...
				m_CharacterWindows[entity].m_DeclareWarText.setFillColor(sf::Color(210, 32, 60));
			...
			if (!characterWindow.m_Shape.getGlobalBounds().contains(mousePosition.x, mousePosition.y))
			{
				...
				if (Map::m_MapUnitData.find(mouseMapPosition) != Map::m_MapUnitData.end())
				{
					...
					if (characterWindow.m_CurrentRegionID != regionID)
					{
						...
						MapRegion& currentMapRegion = Map::GetRegionById(characterWindow.m_CurrentRegionID);
	...
	void ClickButton(CharacterWindow& characterWindow)
	{
		if (InputHandler::GetLeftMouseReleased() && !m_PlayerRegion)
		{
			...
			{
				EntityID warTarget = Map::GetRegionById(characterWindow.m_CurrentRegionID).m_OwnerID;
